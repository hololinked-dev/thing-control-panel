	...
	const updateLocalStorage = useCallback(
		(value: any) => {
			...
			if (!lobj["app-settings"] || !lobj["app-settings"]["useLocalStorage"]) {
				console.info(
					`local storage is disabled, cannot update field ${field} with value ${value}, changes will be lost on refresh`,
				);
				return;
			}
...
export const useAutoCompleteOptionsFromLocalStorage = (field: string) => {
	...
	if (!existingData[field]) existingData[field] = []; // no need to re-render - it will correct at first iteration
	...
	const modifyOptions = useCallback(
		(entry: string | string[], operation: "ADD" | "DELETE") => {
			if (operation === "ADD") {
				if (Array.isArray(entry)) {
					for (let value of entry) {
						if (value) {
							if (!existingData[field].includes(value)) existingData[field].push(value);
						}
					}
				} else if (entry) {
					if (!existingData[field].includes(entry)) existingData[field].push(entry);
				}
			} else {
				if (Array.isArray(entry)) {
					for (let value of entry) {
