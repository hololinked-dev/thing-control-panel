...
export const TabPanel = (props: TabPanelProps) => {
	...
	return (
		...
		>
			{value === index && <Box sx={{ display: "flex", height: "100%" }}>{children}</Box>}
		</div>
	);
};
		...
		<>
			{props.errorMessage ? (
				<Stack>
					...
					{props.errorTraceback
						? props.errorTraceback.map((line: string, index: number) => {
								if (index === 0)
									return (
										<Typography
	...
	return (
		<>
			{showPopout ? (
				...
				>
					<div ref={nwRef}>{props.children}</div>
				</NewWindow>
			) : // </StyleSheetManager>
			null}
		</>
		...
		<>
			...
			<Dialog open={open} onClose={handleClose} fullWidth>
				...
				<DialogActions>
					...
					<Button size="small" onClick={() => setTempValue("")}>
						Clear
					</Button>
