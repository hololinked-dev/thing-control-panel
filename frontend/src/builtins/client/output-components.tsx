...
export const UndockableConsole = observer(() => {
	...
	const [consoleWindowSize, setConsoleWindowSize] = useState<string>(settings.console.defaultWindowSize.toString());
	const [consoleMaxEntries, setConsoleMaxEntries] = useState<string>(settings.console.defaultMaxEntries.toString());
	const [stringifyOutput, setStringifyOutput] = useState<boolean>(settings.console.stringifyOutput);
	...
	const downloadResponse = useCallback(() => {
		consoleEntries ? downloadJSON(consoleEntries, "last-response.json") : console.log("no valid response to download");
	}, [consoleEntries]);
	...
	useEffect(() => {
		const hookedConsole = Hook(
			...
			// @ts-ignore
			(ret) =>
				setConsoleEntries((currValue: any) => {
					if (consoleMaxEntries === "show all") return [ret, ...currValue];
					else if (currValue.length < Number(consoleMaxEntries)) return [ret, ...currValue];
					else return [ret, ...currValue.slice(0, currValue.length - 1)];
				}),
			false,
		);
		return () => {
	...
	return (
		<Stack id="client-output-console-box" sx={{ pt: 1 }}>
			...
			>
				...
				<Stack direction="row">
					<ButtonGroup>
						...
						<IconButton onClick={openLastResponseInNewTab} size="medium" sx={{ borderRadius: 0 }}>
							<OpenInNewTwoToneIcon fontSize="medium" />
						</IconButton>
						<IconButton onClick={downloadLastResponse} size="medium" sx={{ borderRadius: 0 }}>
					...
					<FormControl id="console-window-font-size-form">
						...
						>
							{allowedConsoleFontSizes.map((value: string) => (
								<MenuItem key={"console-window-font-size-selector-" + value} value={value}>
									{value}
								</MenuItem>
							))}
						</Select>
					...
					<FormControl id="console-window-size-form">
						...
						>
							{allowedConsoleWindowSizes.map((value: string) => (
								<MenuItem key={"console-window-size-selector-" + value} value={value}>
									{value}
								</MenuItem>
							))}
						</Select>
					...
					<FormControl id="console-window-max-entries-form">
						...
						>
							{allowedConsoleMaxEntries.map((value: string) => (
								<MenuItem key={"console-window-max-entries-selector" + value} value={value}>
									{value}
								</MenuItem>
							))}
						</Select>
			...
			{undock ? (
				<NewWindow title="title" name="somename" copyStyles={true}>
					<Box sx={{ p: 5 }}>
						<ConsoleOutput
...
export const LiveLogViewer = () => {
	...
	const [logWindowSize, setLogWindowSize] = useState<string>(settings.logViewer.defaultWindowSize.toString());
	const [logOutputFontSize, setLogOutputFontSize] = useState<string>(settings.logViewer.defaultFontSize.toString());
	const [logInterval, setLogInterval] = useState<string>(settings.logViewer.defaultInterval.toString());
	const [logMaxEntries, setLogMaxEntries] = useState<string>(settings.logViewer.defaultMaxEntries.toString());
	...
	const updateLogs = useCallback((data: LogDataType[]) => {
		...
		if (rowDataRef.current !== null) data = [...data, ...rowDataRef.current];
	...
	const fetchLogs = useCallback(async () => {
		try {
			...
			if (response.status !== 200 && response.status !== undefined) {
				if (response.data.exception) thing.setError(response.data.exception.message, response.data.exception.traceback);
				else thing.setError(`could not fetch log events - response code ${response.status}`);
			...
			if (eventSrc) {
				...
				try {
					...
					if (response.status !== 200 && response.status !== undefined) {
						...
						else thing.setError(`could not stop log events - response code ${response.status}`);
						console.log(response);
	...
	return (
		<Stack sx={{ display: "flex", flexGrow: 1, pt: 1, pl: settings.tabOrientation === "vertical" ? 2 : null }}>
			<Stack direction="row" sx={{ pt: 2 }}>
				...
				<ButtonGroup variant="contained" sx={{ pl: 2, pb: 2, pr: 2 }} disableElevation color="secondary">
					<Button sx={{ flexGrow: 0.05, display: "flex" }} onClick={fetchLogs} disabled={eventSrc !== null}>
						Stream
					</Button>
					<Button sx={{ flexGrow: 0.05, display: "flex" }} onClick={stopLogEventSrc} disabled={eventSrc === null}>
						Stop
					</Button>
				</ButtonGroup>
				<Box key="log-window-clear-button-box" sx={{ pb: 2 }}>
					<Button id="log-window-clear-button" variant="outlined" onClick={clearLogs} sx={{ borderColor: "GrayText" }}>
						Clear Logs
					</Button>
				</Box>
				<FormControl id="log-window-interval-form" sx={{ pl: 2 }}>
					...
					>
						{allowedLogIntervals.map((value: string) => (
							<MenuItem key={"log-window-size-selector-" + value} value={value}>
								{value}
							</MenuItem>
						))}
					</Select>
				...
				<FormControl id="log-window-font-size-form" sx={{ pl: 2 }}>
					...
					>
						{allowedConsoleFontSizes.map((value: string) => (
							<MenuItem key={"log-window-font-size-selector-" + value} value={value}>
								{value}
							</MenuItem>
						))}
					</Select>
			...
			{docked ? (
				...
				<NewWindow title={`${thing.td.id}-log-viewer`} name={`${thing.td.id}-log-viewer`}>
...
export const ResponseLogs = observer(() => {
	...
	const logs = thing.lastResponse
		? thing.lastResponse.data
			? thing.lastResponse.data.logs
				? thing.lastResponse.data.logs
				: null
			: null
		: null;
		...
		<>
			{thing.errorMessage ? (
				<Stack id="error-viewer-box-for-padding" sx={{ pt: 1 }}>
					...
					<ErrorViewer errorMessage={thing.errorMessage} errorTraceback={thing.errorTraceback} />
					<Box sx={{ pb: 1 }} />
					<Button
