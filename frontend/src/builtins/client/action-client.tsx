		...
		<Stack id="selected-action-view-layout" sx={{ flexGrow: 1, display: "flex" }}>
			...
			>
				{actionFields.map((name: string) => (
					<Tab key={"selected-action-fields-tab-" + name} id={name} label={name} sx={{ maxWidth: 150 }} />
				))}
			</Tabs>
			{actionFields.map((name: string, index: number) => (
				<TabPanel
					...
					index={index}
				>
					<ActionTabComponents tab={name} action={action} />
				</TabPanel>
			))}
		</Stack>
	);
...
const ActionTabComponents = ({ tab, action }: { tab: string; action: ActionInformation }) => {
	switch (tab) {
		case "Doc":
			return <TDDocViewer resource={action} type="action" />;
		default:
			return <ActionInvokationClient action={action} />;
	}
};

const ActionInvokationClient = ({ action }: { action: ActionInformation }) => {
	...
	const invokeAction = useCallback(async () => {
		try {
			...
			if (http_method.toLowerCase() === "get") {
				...
				if (fetchExecutionLogs) _fullpath += `&fetch_execution_logs=${fetchExecutionLogs}`;
			} else {
				let parsedPayload = JSON.parse(kwargsValue);
				if (parsedPayload) data = parsedPayload;
				else data = null;
				_fullpath = fullpath;
			}
			/* 
            ...
            */
			...
			console.log(
				`ACTION EXECUTION : ${thing.td.title}.${action.name}, REQUEST TIME : ${requestTime}, RESPONSE TIME : ${getFormattedTimestamp()}, EXECUTION TIME : ${executionTime.toString()}ms, RESPONSE BELOW : `,
			);
		} catch (error: any) {
			// console.log(error)
	...
	return (
		<Stack id="action-execution-client-layout" sx={{ pt: 1, flexGrow: 1, display: "flex", pb: 1 }}>
			<InputChoice choice={inputChoice} jsonSchema={action.input} setValue={setKwargsValue} value={kwargsValue} />
			...
			>
				...
				{/* <ButtonGroup> */}
				<Button variant="contained" disableElevation color="secondary" onClick={invokeAction}>
					Execute
				</Button>
