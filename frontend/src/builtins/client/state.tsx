...
export class ResourceInformation {
	...
	get doc(): string {
		if (this.description) return this.description.replace("\n", " ").replace(/\s+/g, " ").trim();
		return "No Docstring Available";
	}

	get state(): string {
		if (!this._state) return "any state";
		let state = "";
		for (let st of this._state) state = state ? state + ", " + st : st;
...
export class PropertyInformation extends ResourceInformation {
	...
	get chips(): string[] {
		...
		for (var key of Object.keys(this.chipKeysSensibleString)) {
			if (this[key as keyof PropertyInformation]) Chips.push(this.chipKeysSensibleString[key]);
		}
		return Chips;
	}
	...
	get inputType(): string {
		...
		else if (this.JSONInputParameterTypes.includes(this.type)) return "code-editor";
		return "RAW";
	}
}
...
export class ThingInformation {
	...
	constructor(info: ThingInfo) {
		...
		for (let key of Object.keys(info.actions)) this.actions.push(new ActionInformation(info.actions[key], key));
		for (let key of Object.keys(info.events)) this.events.push(new EventInformation(info.events[key], key));
...
export class Thing {
	...
	addEventSource(URL: string, src: EventSource | string) {
		if (this.eventSources[URL]) {
			if (this.eventSources[URL] instanceof EventSource) console.warn("Event Source already exists for this URL");
		}
		this.eventSources[URL] = src;
	}

	removeEventSource(URL: string) {
		if (this.eventSources[URL]) delete this.eventSources[URL];
	}

	removeNodeWoTEventSource(name: string) {
		if (
			this.client.observedProperties.size > 0 &&
			this.client.observedProperties.get(name).client.activeSubscriptions.size > 0
		) {
	...
	get logEventsPusherURL(): string {
		return "/resources/log-events";
	}

	get logEventsStopURL(): string {
		return "/resources/log-events";
	}

	get logEventsURL(): string {
		return "/resources/log-events";
	}
}
