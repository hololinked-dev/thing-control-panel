...
const FunctionalitiesView = observer(() => {
	...
	const removeDuplicateWindow = useCallback(
		(index: number) => {
...
const Functionalities = observer(({ type }: { type: string }) => {
	switch (type) {
		case "Doc/Description":
			return <ClassDocWindow />;

		case "Database":
			return <Typography sx={{ p: 2 }}>No DB client</Typography>;

		//  case 'Log Viewer' : return <LiveLogViewer />

		default:
			return <InteractionAffordancesView type={type as "Properties" | "Actions" | "Events"} />;
	}
});
...
const InteractionAffordancesView = observer(({ type }: { type: "Properties" | "Actions" | "Events" }) => {
	...
	return (
		<Stack direction="row" sx={{ flexGrow: 1, overflowX: { xs: "scroll", sm: "auto" }, overflowY: "hidden" }}>
			...
			>
				<List id="interaction-affordance-objects-list" dense disablePadding>
			...
			>
				{objects[selectedIndex] ? <InteractionAffordanceSelect object={objects[selectedIndex]} type={type} /> : null}
			</Box>
		</Stack>
	);
});

const InteractionAffordanceSelect = ({ object, type }: { object: ResourceInformation; type: string }) => {
	switch (type) {
		case "Events":
			return <SelectedEventWindow event={object as EventInformation} />;
		case "Actions":
			return <SelectedActionWindow action={object as ActionInformation} />;
		default:
			return <SelectedPropertyWindow property={object as PropertyInformation} />;
	}
};
    ...
    */
	useEffect(() => {
		const startServient = async () => {
			...
			const IsOurWebsite =
				window.location.hostname.endsWith("hololinked.dev") || window.location.hostname.endsWith("hololinked.net");
			...
			try {
				if ((IsOurWebsite && IsSSLWebsite) || (!IsOurWebsite && appConfig.useSSL)) {
					...
					console.log(
						"added HTTPS and WSS clients, HTTP & WS (non-SSL version) clients not supported although Thing Description may be fetched",
					);
				} else {
					// @ts-expect-error
					...
					console.log(
						"added non-SSL HTTP and WS client factories, HTTPS and WSS (SSL version) clients not supported although Thing Description may be fetched",
					);
				}
		...
		<ThingContext.Provider value={thing.current}>
			<Box id="client-layout-box" sx={{ pt: 0.5 }}>
				<Stack id="thing-viewer-page-layout">
					<Locator />
					{showSettings ? <AppSettings globalState={null} /> : <ThingViewer />}
				</Stack>
