		...
		<Stack id="selected-property-layout" sx={{ flexGrow: 1, display: "flex" }}>
			...
			>
				{propertyOptions.map((name: string) => {
					return <Tab key={"selected-property-options-tab-" + name} id={name} label={name} sx={{ maxWidth: 150 }} />;
				})}
			</Tabs>
...
export const PropertyTabComponents = ({ tab, property }: { tab: string; property: PropertyInformation }) => {
	switch (tab) {
		case "Execute":
			return <RWO property={property}></RWO>;
		default:
			return <TDDocViewer resource={property} type="property"></TDDocViewer>;
	}
};
...
export const RWO = ({ property }: { property: PropertyInformation }) => {
            ...
            */
                ...
                */
					if (mode === "READ") {
						...
						response = await thing.client.writeProperty(
							property.name,
					...
					if (response.data && response.data.exception)
						...
						if (response.data) consoleOutput = response.data;
						thing.resetError();
					}
				}
				if (settings.console.stringifyOutput) console.log(JSON.stringify(consoleOutput, null, 2));
			...
			console.log(
				`PROPERTY ${mode} : ${property.name}, REQUEST TIME : ${requestTime}, RESPONSE TIME : ${getFormattedTimestamp()}, EXECUTION TIME : ${executionTime.toString()}ms, RESPONSE BELOW :`,
			);
		},
	...
	return (
		<Stack id="property-rw-client" sx={{ pt: 1, flexGrow: 1, display: "flex" }}>
			...
			<Stack id="property-rw-client-options-layout" spacing={1} useFlexGap direction="row" sx={{ flexWrap: "wrap" }}>
				{property.readOnly ? null : (
					<FormControl>
						<RadioGroup id="input-choice-group" row value={inputChoice} onChange={handleInputSelection}>
				...
				<ButtonGroup id="rw-buttons" variant="contained" disableElevation color="secondary">
					<Button onClick={readProp}>Read</Button>
					<Button disabled={property.readOnly} onClick={writeProp}>
						Write
					</Button>
				</ButtonGroup>
				{property.observable ? (
...
const Observe = observer(
	({ property, skipDataSchemaValidation }: { property: PropertyInformation; skipDataSchemaValidation: boolean }) => {
		...
		const observeProp = useCallback(() => {
			if (clientChoice === "node-wot") {
				thing.client
					.observeProperty(property.name, async (data: any) => {
						try {
							data.ignoreValidation = skipDataSchemaValidation;
							const value = await data.value();
							if (settings.console.stringifyOutput) console.log(`${property.name} change event - ${value}`);
							else console.log(`${property.name} change event - ${JSON.parse(value)}`);
						} catch (error) {
				...
				source.onmessage = (event: MessageEvent) => {
					if (settings.console.stringifyOutput) console.log(event.data);
		...
		return (
			<ButtonGroup id="observe-buttons" variant="contained" disableElevation color="secondary">
				<Button disabled={thing.eventSources[property.name] !== undefined} onClick={observeProp}>
					Observe
				</Button>
				<Button disabled={!thing.eventSources[property.name]} onClick={unobserveProp}>
					Stop
				</Button>
			</ButtonGroup>
		);
