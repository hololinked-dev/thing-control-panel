...
export const Subscription = observer(({ event }: { event: EventInformation }) => {
	...
	const streamEvent = useCallback(() => {
		if (clientChoice === "node-wot") {
			thing.client
				.subscribeEvent(event.name, async (data: any) => {
					data.ignoreValidation = skipDataSchemaValidation;
					const value = await data.value();
					if (settings.console.stringifyOutput) console.log(value);
	...
	return (
		<Stack sx={{ flexGrow: 1, display: "flex" }}>
			<Link
				...
				sx={{ display: "flex", alignItems: "center", cursor: "pointer", pt: 1, fontSize: 18, color: "#0000EE" }}
				underline="hover"
				variant="caption"
			>
				{eventURL}
			...
			<Stack direction="row" sx={{ pt: 1 }}>
				<ButtonGroup variant="contained" sx={{ pr: 2 }} disableElevation color="secondary">
					<Button
		...
		<Stack id="selected-action-view-layout" sx={{ flexGrow: 1, display: "flex" }}>
			...
			{eventFields.map((name: string, index: number) => (
				<TabPanel
					...
					index={index}
				>
					<EventTabComponents tab={name} event={event} />
				</TabPanel>
			))}
		</Stack>
	);
...
const EventTabComponents = ({ tab, event }: { tab: string; event: EventInformation }) => {
	const thing = useContext(ThingContext) as Thing;

	switch (tab) {
		case "Doc":
			return <TDDocViewer resource={event} type="event" />;
		default:
