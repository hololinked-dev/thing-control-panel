...
export const SampleRowData = [
	...
	},
	{
		...
		level: "DEBUG",
		message:
			"This is a yet another somewhat unnecessarily made long debug message with garbage - lkajsdhflksjadhfljksadhjfkl hjklsfhkjlsahfjklhsdjklfhjklsafjklhflj kljahsdlfkjhsadlkjfh lkjsdh ljklsdhafjklhsadlkfhsdajklfhjklas lk sdjfklhasljkhflsadh ",
	},
	{
		id: 4,
		timestamp: "20:12",
	...
	},
	{
		...
		level: "INFO",
		message:
			"This is a yet another somewhat unnecessarily made info debug message with garbage - lkajsdhflksjadhfljksadhjfkl hjklsfhkjlsahfjklhsdjklfhjklsafjklhflj kljahsdlfkjhsadlkjfh lkjsdh ljklsdhafjklhsadlkfhsdajklfhjklas lk sdjfklhasljkhflsadh ",
	},
	{
		id: 7,
		timestamp: "20:13",
...
export const PinnableCell = (props: any) => {
	const buttonClicked = () => {
		...
		let currentData =
			props.api.pinnedRowModel.pinnedTopRows.length > 0
				? props.api.pinnedRowModel.pinnedTopRows.map((data: any) => data.data)
				: [];
	...
	return (
		<span>
			<ButtonGroup>
				<IconButton onClick={buttonClicked} sx={{ borderRadius: 0, flexGrow: 1, display: "flex" }}>
...
export const ROTimestampCell = (props: any) => {
	...
	return (
		<Typography fontSize={props.column.colDef.cellStyle.fontSize} fontFamily="monospace" variant="overline">
			{cellValue.split("T")[1]}
		</Typography>
	);
};
...
export const ColoredBackendMessage = (props: any) => {
	...
	return (
		<Typography
			...
			fontSize={props.column.colDef.cellStyle.fontSize}
			// @ts-ignore
			sx={{ color: logColorLevels[props.data.level] ? logColorLevels[props.data.level] : "black", pt: 0.5, pb: 0.5 }}
		>
			{cellValue}
		</Typography>
	);
...
export const LogTable = (props: LogTableProps) => {
	...
	useEffect(() => {
		...
		if (_rowData) {
			for (let i = 0; i < _rowData.length; i++) _rowData[i].id = i;
		}
		setRowData(_rowData);
		...
		<>
			{rowData ? (
				<Stack sx={{ flexGrow: 1, display: "flex", minHeight: props.minHeight ? props.minHeight : "500px" }}>
					...
					<Box className="ag-theme-material" style={{ width: "100%", height: "100%" }} sx={{ pt: 2 }}>
						<AgGridReact
							// @ts-ignore
							ref={gridRef}
							...
							rowData={rowData}
							getRowId={(params) => {
								console.log("id", String(params.data.id));
...
export const useRemoteObjectLogColumns = (fontSize: string = "16px") => {
	const columnDefs = useMemo(
		() => [
			...
			{
				colId: "action-buttons",
				headerName: "ACTIONS",
				maxWidth: 100,
				cellRenderer: PinnableCell,
			},
		],
		[fontSize],
	);
		...
		() => [
			{
				...
				maxWidth: 300,
				suppressSizeToFit: true,
				cellRenderer: WebGUITimestampCell,
				cellStyle: { fontSize: fontSize },
				resizable: true,
				filter: true,
				floatingFilter: true,
