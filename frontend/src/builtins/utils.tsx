...
export async function asyncRequest(AxiosObject: AxiosRequestConfig) {
	const response = await axios(AxiosObject)
		.then((response) => {
			if (AxiosObject.responseType !== "blob") response.status = response.data.responseStatusCode;
			return response;
		})
		.catch((error) => {
			return { error: error };
...
export const parseWithInterpretation = (value: any, interpretation: string) => {
	...
	switch (interpretation.toLowerCase()) {
		...
		case "boolean": {
			...
			else if (typeof jsonValue === "number") return Boolean(Number(jsonValue));
			else if (typeof jsonValue === "boolean") return jsonValue;
		...
		default:
			return jsonValue; // String, Bytes, IPAddress,
		// object & array?
	}
...
export const parseActionPayloadWithInterpretation = (value: any, schema: any) => {
	if (schema.type === "object" && schema.properties && Object.keys(schema.properties).length === 1) {
		let obj = {};
		obj[Object.keys(schema.properties)[0]] = parseWithInterpretation(
			value,
			schema.properties[Object.keys(schema.properties)[0]].type,
		);
		return obj;
	} else console.log("use code editor to supply input for this action");
};
