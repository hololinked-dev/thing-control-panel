...
import {
	allowedConsoleFontSizes,
	allowedConsoleMaxEntries,
	allowedConsoleWindowSizes,
	allowedLogIntervals,
} from "./client/output-components";
...
const EditableTextSetting = observer(
	({
		settingName,
		settingURL,
		initialValue,
					...
					startAdornment={
						<InputAdornment position="start">
							{edit ? (
								...
								<IconButton sx={{ bgcolor: "#808080", borderRadius: 0 }} onClick={() => setEdit(true)}>
...
const BooleanSwitchSetting = observer(
	({
		label,
		initialValue,
		settingName,
...
const BooleanCheckboxSetting = observer(
	({
		label,
		initialValue,
		settingName,
			...
			<FormControl id={id + "-form"}>
				...
				>
					{allowedValues.map((value: string, index: number) => (
						<MenuItem key={`$${id}-selector-${value}-at-pos-${index}`} value={value}>
							{value}
						</MenuItem>
					))}
				</Select>
...
const ThingViewerSettings = () => {
	...
	return (
		<Box>
			...
			<Grid
				id="remote-object-viewer-console-settings"
				spacing={3}
				container
				direction="row"
				sx={{ flexWrap: "wrap", pt: 2, "& > *": { pb: 2 } }}
			>
				<Grid item>
					<SelectSetting
			...
			<Grid container direction="column" id="dashboards-settings" spacing={2} sx={{ pt: 2 }}>
				...
				<Grid item>
					<BooleanSwitchSetting
						...
						tooltip={`uses your browser's local storage to save settings and app data (like saved links to thing descriptions) 
                            permanently, nothing is transferred or stored somewhere else.
                            If you do not enable this, all settings and app data will be lost on page refresh.`.replace(
							/\s+/g,
							" ",
						)}
					/>
...
const ClearLocalStorageButton = () => {
	...
	return (
		<>
			<Button sx={{ alignSelf: "flex-start" }} onClick={() => setOpen(true)} variant="contained" color="warning">
				Clear Local Storage
			</Button>
			<Dialog open={open} onClose={() => setOpen(false)}>
				<DialogTitle>Confirm</DialogTitle>
				<DialogContent>
					<DialogContentText>Are you sure you want to clear local storage?</DialogContentText>
				</DialogContent>
				<DialogActions>
...
export const AppSettings = ({ globalState }: { globalState: any }) => {
	...
	const updateSettingsInStorage = useCallback(
		async (URL: string, settingName: string, value: any, event: React.BaseSyntheticEvent | any) => {
			...
			if (globalState) {
				try {
					...
					if (response.status === 200) globalState.updateSetting(settingName, value);
