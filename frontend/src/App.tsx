...
const OnLoadMessage = () => {
	...
	const handleClose = (event?: React.SyntheticEvent, reason?: string) => {
		...
		if (settings.useLocalStorage) updateLocalStorage({ ...settings, showWebsiteWarningAgain: showWarningAgain });
		setOpen(false);
	};

	if (!settings.showWebsiteWarningAgain) return null;
			...
			<Alert onClose={handleClose} severity="warning" sx={{ width: "100%" }}>
				...
				{settings.useLocalStorage ? (
					<FormControlLabel
						...
						control={<Checkbox size="small" onChange={(event) => setShowWarningAgain(!event.target.checked)} />}
